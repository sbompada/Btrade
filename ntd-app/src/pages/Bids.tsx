...
function BidTicket({ issue, onClose, onComplete }: { issue: BidIssue; onClose: () => void; onComplete: () => void }) {
  const { token } = useAuth();
  const [lots, setLots] = useState(1);
  const [price, setPrice] = useState(issue.priceHigh ?? 0);
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError(null);
    try {
      await bidsApi.place(token, { issueId: issue.id, lots, price });
      onComplete();
    } catch (cause) {
  ...
  return <div className="order-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="order-ticket" role="dialog" aria-modal="true" aria-labelledby="bid-title">
      <div className="order-ticket-head"><div><strong id="bid-title">Apply for {issue.name}</strong><span>{issue.exchange} · {issue.code}</span></div><button onClick={onClose} aria-label="Close bid ticket">×</button></div>
      <form onSubmit={submit}>
        <div className="order-fields">
          ...
          <label><span>Bid price</span><input aria-label="Bid price" type="number" min={issue.priceLow ?? 0} max={issue.priceHigh ?? 0} step="0.01" value={price} onChange={(event) => setPrice(Number(event.target.value))} /></label>
        </div>
        <div className="bid-ticket-summary"><span>{quantity} shares</span><span className="num">₹{num(quantity * price)}</span></div>
        ...
        <button className="btn primary block" disabled={submitting || lots < 1 || price <= 0}>{submitting ? 'Submitting…' : 'Submit application'}</button>
      </form>
    </section>
  </div>;
}
...
export default function Bids() {
  ...
  return <AppShell tabs={['IPOs', 'My applications']} activeTab={view === 'issues' ? 'IPOs' : 'My applications'} onTabChange={(tab) => setView(tab === 'IPOs' ? 'issues' : 'applications')}>
    <div className="grid-4">
      ...
      <div className="tile"><span className="tile-label">UPCOMING</span><span className="tile-value num">{issues.filter((issue) => issue.status !== 'open').length}</span><span>Announced issues</span></div>
    ...
    <section className="panel">
      ...
      {loading ? <div className="stub"><span>Loading IPO bids…</span></div> : view === 'issues' ? <div className="bid-grid">{filteredIssues.map((issue) => {
        ...
        return <article className="bid-card" key={issue.id}><div className="bid-card-head"><div><strong>{issue.name}</strong><span>{issue.code} · {issue.exchange}</span></div><span className={`tag calendar-status ${issue.status}`}>{issue.status}</span></div><div className="bid-terms"><div><span>PRICE BAND</span><strong className="num">{issue.priceLow ? `₹${num(issue.priceLow)}–₹${num(issue.priceHigh ?? 0)}` : 'Awaited'}</strong></div><div><span>LOT SIZE</span><strong className="num">{issue.lotSize ? `${issue.lotSize} shares` : 'Awaited'}</strong></div><div><span>CLOSES</span><strong>{issue.closeDate ?? 'TBA'}</strong></div></div><button className={application ? 'btn ghost block' : 'btn primary block'} disabled={issue.status !== 'open' || !!application} onClick={() => setTicket(issue)}>{application ? 'Application submitted' : issue.status === 'open' ? 'Apply' : 'Not open yet'}</button></article>;
      })}{!filteredIssues.length && <div className="stub"><span>No IPOs match “{query}”.</span></div>}</div> : bids.length ? <div className="bid-applications"><div className="bid-application-head"><span>ISSUE</span><span>QTY</span><span>PRICE</span><span>AMOUNT</span><span>STATUS</span><span /></div>{bids.map((bid) => <div className="bid-application-row" key={bid.id}><div><strong>{bid.name}</strong><span>{bid.code} · {bid.lots} lot{bid.lots === 1 ? '' : 's'}</span></div><span className="num">{bid.quantity}</span><span className="num">₹{num(bid.price)}</span><span className="num">₹{num(bid.amount)}</span><span className={`tag bid-status ${bid.status.toLowerCase()}`}>{bid.status}</span><button className="chip" disabled={bid.status !== 'SUBMITTED'} onClick={() => void cancel(bid)}>Cancel</button></div>)}</div> : <div className="stub"><strong>No applications yet</strong><span>Apply to an open IPO to see it here.</span></div>}
    </section>
    {ticket && <BidTicket issue={ticket} onClose={() => setTicket(null)} onComplete={() => { setTicket(null); setView('applications'); void load(); }} />}
  </AppShell>;
}